...
├── packages/
│   └── core/           # Regras puras, IA Engine (SOAP), Evolution API client e schemas MySQL
...
4. **Deploy Vercel + Hostinger:** O projeto `clinica-viver-web` é construído pela raiz do monorepo e deve servir esta aplicação diretamente. Nunca adicionar um rewrite global de `/:path*`; somente as rotas de API específicas podem seguir para `app.clinicavivermais.cloud`.
