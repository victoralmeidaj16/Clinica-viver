...
export async function POST(request: Request) {
  try { const context = await resolveRequestContext(request, true); const body = await readJson(request); return success(await createAppointmentFlow(context, parseAppointmentInput(body, context), body.chargeDueAt ? String(body.chargeDueAt) : undefined, typeof body.serviceKey === 'string' ? body.serviceKey : undefined), 201); }
  catch (error) { return failure(error); }
}
