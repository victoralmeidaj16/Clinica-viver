...
import {
  ArrowRight, CalendarClock, Check, Copy, CreditCard, FileText, Loader2,
} from 'lucide-react';
      ...
      {!payment ? (
        <form onSubmit={generate} className="card space-y-5">
          <div>
            ...
            <p className="mt-1 text-xs text-muted">Informe o CPF para confirmar que esta cobrança pertence ao seu atendimento.</p>
          </div>
          ...
          <button type="submit" disabled={loading} className="btn-accent w-full py-4 text-sm">
            {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : <>Continuar para pagamento <ArrowRight className="h-5 w-5" /></>}
          </button>
        </form>
      ) : (
        <section className="card space-y-5 text-center">
          ...
          {payment.invoiceUrl && <a href={payment.invoiceUrl} target="_blank" rel="noreferrer" className="btn-outline w-full py-3 text-sm">Abrir opções de pagamento</a>}
        </section>
      )}
    </div>
  );
