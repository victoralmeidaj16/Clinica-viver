...
import { Banknote, Check, Copy, ExternalLink, FileCheck2, Loader2, Trash2 } from 'lucide-react';
...
export function FaturaNfsePanel({ convenioId, fatura, onRefresh }: { convenioId: string; fatura: FaturaConvenioView; onRefresh: () => Promise<void> }) {
  const [working, setWorking] = useState<'boleto' | 'nfse' | 'cancelar_fatura'>();
  const [fiscal, setFiscal] = useState<FiscalData>();
  const [error, setError] = useState('');
  const [copied, setCopied] = useState(false);
  const [cancelling, setCancelling] = useState(false);
  const [cancelReason, setCancelReason] = useState('');
  const [confirmingCancelFatura, setConfirmingCancelFatura] = useState(false);
  ...
  const cancelFatura = async () => {
    setWorking('cancelar_fatura'); setError('');
    try {
      await applicationRequest(`${base}/cancelar`, {
        method: 'POST', headers: commandHeaders(),
      });
      setConfirmingCancelFatura(false);
      await onRefresh();
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : 'Não foi possível cancelar a fatura.');
    } finally {
  ...
  return (
    <article className="rounded-2xl border border-psi-soft/70 bg-white p-4 shadow-sm">
      ...
      {confirmingCancelFatura && (
        <div className="mt-3 flex flex-col gap-2 rounded-xl border border-rose-200 bg-rose-50 p-3">
          <p className="text-xs font-bold text-rose-900">
            Tem certeza que deseja cancelar esta fatura? Os atendimentos vinculados voltarão imediatamente para &ldquo;A faturar&rdquo; para que você possa selecioná-los novamente.
          </p>
          <div className="flex gap-2">
            <button
              type="button"
              onClick={() => setConfirmingCancelFatura(false)}
              className="btn-outline px-3 py-2 text-[11px] bg-white"
            >
              Não cancelar
            </button>
            <button
              type="button"
              onClick={() => void cancelFatura()}
              disabled={working === 'cancelar_fatura'}
              className="rounded-xl bg-rose-700 px-3 py-2 text-[11px] font-black text-white hover:bg-rose-800 transition disabled:opacity-50 flex items-center gap-1.5"
            >
              {working === 'cancelar_fatura' ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Trash2 className="h-3.5 w-3.5" />}
      ...
      {cancelling && <div className="mt-3 flex flex-col gap-2 rounded-xl border border-rose-200 bg-rose-50 p-3"><label className="text-xs font-bold text-rose-900">Motivo do cancelamento da NFS-e<input value={cancelReason} onChange={(event) => setCancelReason(event.target.value)} className="input mt-1 bg-white text-xs" /></label><div className="flex gap-2"><button type="button" onClick={() => setCancelling(false)} className="btn-outline px-3 py-2 text-[11px]">Voltar</button><button type="button" onClick={() => void cancelNfse()} disabled={!cancelReason.trim() || Boolean(working)} className="rounded-xl bg-rose-700 px-3 py-2 text-[11px] font-black text-white disabled:opacity-50">Confirmar cancelamento</button></div></div>}
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        {fatura.status !== 'paga' && !confirmingCancelFatura && (
          <button
            type="button"
            onClick={() => setConfirmingCancelFatura(true)}
            disabled={Boolean(working)}
            className="rounded-xl border border-rose-200 bg-rose-50/70 px-3 py-2 text-[11px] font-bold text-rose-700 hover:bg-rose-100 hover:border-rose-300 transition flex items-center gap-1.5 disabled:opacity-50"
          >
            <Trash2 className="h-3.5 w-3.5" /> Cancelar fatura
          </button>
        )}
      </div>
    </article>
