      ...
      <div className="grid grid-cols-3 gap-2">
        {TURNOS_ATENDIMENTO.map((turno) => {
          ...
          return (
            <label
              key={turno.value}
              className={`group cursor-pointer rounded-2xl border text-center transition-all ${compact ? 'p-2.5' : 'p-3'} ${
                selected
                  ? 'border-psi-vibrant bg-psi-vibrant/10 text-psi-deep shadow-sm ring-1 ring-psi-vibrant/20'
                  : 'border-line bg-white text-muted hover:border-psi-vibrant/50 hover:bg-psi-soft/30'
              }`}
            >
              <input
                type="radio"
