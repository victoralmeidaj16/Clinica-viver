...
export function NfseIntegrationCard() {
  ...
  const prontoParaEmitir = certificado?.certificado.apto === true && convenioRespondeu;
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className={`rounded-full px-2.5 py-1 text-[10px] font-extrabold ${
                prontoParaEmitir ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-900'
              }`}>
                {carregando
                  ? 'VERIFICANDO'
                  : prontoParaEmitir
                    ? restrito ? 'APTO PARA HOMOLOGAÇÃO' : 'APTO PARA EMISSÃO'
                    : 'AÇÃO NECESSÁRIA'}
              </span>
            </div>
