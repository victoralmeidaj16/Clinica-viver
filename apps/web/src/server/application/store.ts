import {
  InMemoryAppointmentRepository, InMemoryCommunicationAudit, InMemoryIdentityRepository,
...
function createState(): DemoApplicationState {
  ...
  const membership: OrganizationMembership = { id: 'membership-demo', organizationId: organization.id, userId: professional.userId, roles: ['professional'], status: 'active', professionalProfileId: professional.id, createdAt, updatedAt: createdAt };
  const patients = [
  ...
  const charge = createFinancialCharge({ id: 'charge-1', organizationId: organization.id, sessionId: 'session-1', patientId: 'patient-1', professionalId: professional.id, issuedAt: '2026-07-30T12:00:00.000Z', dueAt: '2026-08-05T23:59:00.000Z', amountCents: 25000, createdAt: '2026-07-30T12:00:00.000Z' });
  return {
